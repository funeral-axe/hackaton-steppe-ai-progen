...
def get_best_whisper_model(model_name: str):
    ...
    Explicit environment configuration has priority.
    Otherwise CUDA is selected automatically with CPU fallback.
    """
    requested_device = (
    ...
    if requested_device:
        ...
        print(
            "[word-search] Explicit Whisper runtime: "
    ...
    try:
        ...
        print(
            "[word-search] CUDA detection failed: "
    ...
    if gpu_count > 0:
        print(
            "[word-search] NVIDIA GPUs detected: "
            f"{gpu_count}"
        )
        ...
        try:
            ...
            print(
                "[word-search] Whisper uses NVIDIA GPU"
            )
            ...
            print(
                "[word-search] GPU initialization failed; "
                "falling back to CPU: "
                f"{type(gpu_error).__name__}: "
                f"{gpu_error}"
            )

    print(
        "[word-search] Whisper uses CPU"
    )
